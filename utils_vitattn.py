...
def Reinhard(img_arr):
    standard_img = "/scratch/users/k21066795/NBT-Classifier/data/he.jpg"
...
def plot_multiple(img_list, caption_list=None, grid_x=4, grid_y=4, 
                 figure_size=(10, 10), cmap = "hot", save_pt=None):
    ...
    for i, img in enumerate(img_list):
        ...
        if len(img.shape) == 2 or (len(img.shape) == 3 and img.shape[2] == 1):
            axes[i].imshow(img.squeeze(), cmap=cmap if cmap else 'hot')
        else:
            axes[i].imshow(img)
        axes[i].axis('off')
        if caption_list:
    ...
    plt.tight_layout()
    if save_pt:
        plt.savefig(save_pt, bbox_inches='tight', dpi=300, facecolor='white')
